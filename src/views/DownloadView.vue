...
const reloadDownload = async () => {
  ...
  for (const key in selectedTask) {
    ...
    const videoType = checkUrl(item.url, 'video')
    // const { body, url } = await checkUrlRedirect(item.url, 'video')
    const info: any = await checkUrlRedirect(item.url, 'video')

    const videoInfo: any = await parseHtml(info.body, videoType, info.url, 'video')
